...
graph_list <- lapply(very_short_list, function(hsa_name){
  mapkG <- try(
  {
    ...
    mapkG <- parseKGML2Graph(tmp,expandGenes=F)
    mapkG
  })  
})
...
all_parents <- lapply(unique_symbol_names, gene_iterator)
names(all_parents) <- unique_symbol_names
